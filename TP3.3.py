...
#used to read and write files for High Scores Section
...
#gets the list of songs in the directory of the file
def getSonglist():
...
#Uses the tempo method of the Aubio module to analyze a wav file and return data (list of beat times)
...
#physics calculations/algorithm - converted beats to periods then frequency then pitch
...
#Matches the beats list to the list of pitches
#classifies into different type of game beats based on calculated pitches
#returns new list to fit game specifications
...
#plays Music (from start) using the mixeer method of the pygame module 
def playMusic(mode, song):
    mixer.init()
    mixer.music.load(song)
    mixer.music.play()

#loads Music using the mixeer method of the pygame module 
...
#plays song (from already initialized song) using the mixeer method of the pygame module 
def playSong(mode, song):
    return mixer.music.play()


#stops song using the mixeer method of the pygame module 
...
class Home(Mode):
    ...
    #Selects active mode when user clicks on enter image
...
class Selection(Mode):
    ...
    #allows users to use keyboard to choose between songs in directory
...
class SelectMode(Mode):
    ...
    #Selects game mode when user clicks on respective images
    def mousePressed(mode, event):
...
class GameComputerPlayer(GameTwoPlayer):
    ...
    #disables a, d, w keypresses for computer player
    def keyPressed(mode, event):
        if(event.key == 'a'):
            return
        if(event.key == 'd'):
    ...
    #AI algorithm (recursive) to optimize computer player movement to collect beats
    def collectAI(mode, currLane, targetLane):
        if(currLane == targetLane):
            return
        else:
            direction = "none"
            if(currLane > targetLane):
                currLane -= 1
                direction = "left"
            elif(currLane < targetLane):
                currLane += 1
                direction = "right"
            mode.moveComp(direction)
            mode.collectAI(currLane, targetLane)

    #AI algorithm (recursive) to optimize computer player movement to avoid obstacles
    #AI algorithm (recursive) for computer to track player's movements and choose between several paths to sabatoge the player (push them into obstacles)
    def avoidAI(mode, currLane, targetLane, playerLane):
        if(currLane == targetLane):
            mode.jumpOver()
        elif(playerLane == targetLane):
            return
        else:
            direction = "none"
            if(currLane < playerLane and playerLane < targetLane):
                currLane += 1
                direction = "right"
            elif(currLane > playerLane and playerLane > targetLane):
                currLane -= 1
                direction = "left"
            else:
                return
            mode.moveComp(direction)
            playerLane = mode.getPlayerLane()
            mode.avoidAI(currLane, targetLane, playerLane)

    #moves the computer by the direction specified
    def moveComp(mode, direction):
        if(direction == "left"):
            mode.position2x -= 200
            if(mode.getPlayerCollision(2) == True):
                mode.positionx -= 200
    ...
    #gets the lane of the computer AI player
    def getCurrentLane(mode):
        if(mode.position2x == mode.width//2 - 200):
            return 1
        elif(mode.position2x == mode.width//2 - 400):
            return 0
    ...
    #gets the lane of the player
    def getPlayerLane(mode):
        if(mode.positionx == mode.width//2 - 200):
            return 1
        elif(mode.positionx == mode.width//2 - 400):
            return 0
        elif(mode.positionx == mode.width//2 + 200):
            return 3
        elif(mode.positionx == mode.width//2 + 400):
            return 4
        else:
    ...
    #AI movement of the computer player - calls upon recursive functions for computer to optimize movement and player tracking
    def moveComputer(mode):
        if((len(mode.nextAll) > 0)):
            if(mode.frozenTwo == False and mode.jumpTwo == False):
                ...
                column = nextItem[1]
                targetLane = column
                currPosition = mode.position2x
                currLane = mode.getCurrentLane()
                if(name == "ice" or name == "spike"):
                    playerLane = mode.getPlayerLane()
                    mode.avoidAI(currLane, targetLane, playerLane)
                else:
                    mode.collectAI(currLane, targetLane)
    ...
    #calls computer player to jump (used if an obstacle is detected in path)
    def jumpOver(mode):
        mode.position2y = mode.height*(4/5) - 150
        mode.jumpTwo = True        
...
class Statistics(Mode):
    ...
    #calculates the total percentage of red beats collected by players
    ...
    #calculates the total percentage of green beats collected by players
    ...
    #calculates the total percentage of blue beats collected by players
    ...
    #algorithm to return the average time between the beats missed and the last beat collected
    #provides statistics for the player to improve performance
    ...
    #algorithm to return the average distance between the beats missed and the last beat collected
    #provides statistics for the player to improve performance
...
class HighScores(Mode):
    ...
    #returns the highest score to see which player puts their name on leaderboard (only used when mode selected is two player)
    ...
    #allows users to start typing their name when they click on the textbook
    def mousePressed(mode, event):
        if(event.x > 100 and event.x < (mode.height-100) and event.y > mode.height//2 -25 and event.y < mode.height//2 + 25):
            mode.typing = True

    #creates a text entry input box for users to enter the names - includes various special characters, capitalization, spaces, and backspace
    def keyPressed(mode, event):
        if(event.key == "Enter"):
